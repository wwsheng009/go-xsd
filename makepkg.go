...
type PkgBag struct {
	...
	longNameMap  map[string]string //用于判断长名称是否唯一
	shortNameMap map[string]string //用于判断短名称是否唯一，相对于长名称来说，短名称可读性更好
	nameCount    int               //短名称记数器
}
...
func newPkgBag(schema *Schema) (bag *PkgBag) {
	...
	bag.imports[bag.impName] = "github.com/wwsheng009/go-xsd/types"
	...
	bag.addType(nil, idPrefix+"_Cdata_", "").addField(nil, idPrefix+"CDATA", "string", ",chardata")
...
func (me *declMethod) render(bag *PkgBag, dt *declType) {
	...
	bag.appendFmt(true, "func (me %s) %s %s { %s }",
		bag.rewriteTypeSpec(me.ReceiverType),
		ustr.Ifs(strings.Contains(me.Name, "("),
			me.Name,
			me.Name+" ()"),
		rt, strings.Replace(me.Body, me.ReturnType, rt, -1))
}
	...
	if !me.rendered {
		...
		if me.checkForEquivalents(bag); len(me.EquivalentTo) == 0 {
			...
			if len(me.Type) > 0 {
				...
				bag.appendFmt(true, "}")
				if PkgGen.AddWalkers && !strings.HasPrefix(myName, idPrefix+AttrPrefix) {
